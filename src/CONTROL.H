   ...
   if (Md->Pl[i].Action == ACTION_SNIPER)
    {
     ...
     if (Suunta(Md->Pl[i].Dir-itofix(64)) >= itofix(128)) Md->Pl[i].S = 1; else Md->Pl[i].S = 0;
    } // if (Md->Pl[i].Action == ACTION_SETPLATE)
  } else// live
   {Md->Pl[i].Sx-=1;if (Md->Pl[i].Sx<0) Md->Pl[i].Sx = 0; }
 } // PlayerLeft(int i)
void PlayerRight(int i)
 {
  ...
  if (Md->Pl[i].Menu == -1) // jos menu ei ole aktiivinen
   ...
   {Md->Pl[i].Sx+=1;if (Md->Pl[i].Sx >= Lev->Mapsizex-Clipx) Md->Pl[i].Sx = Lev->Mapsizex-Clipx-1; }
 } // PlayerRight(int i)
void PlayerUp(int i)
 {
  if (Md->Pl[i].Live)
  ...
  if (Md->Pl[i].Menu == -1) // jos menu ei ole aktiivinen
   {
    ...
    Md->Pl[i].Dir = Suunta(Md->Pl[i].Dir+itofix(1)); else
   ...
   {Md->Pl[i].Sy-=1;if (Md->Pl[i].Sy<0) Md->Pl[i].Sy = 0; }
...
void PlayerDown(int i)
 {
  ...
  if (Md->Pl[i].Menu == -1) // jos menu ei ole aktiivinen
   ...
   {Md->Pl[i].Sy+=1;if (Md->Pl[i].Sy >= Lev->Mapsizey-Clipy) Md->Pl[i].Sy = Lev->Mapsizey-Clipy-1; }
 } // PlayerDown(int i)
...
void PlayerUse(int i)
 {
  ...
  if (Md->Pl[i].Live) if (Md->Pl[i].Reloading == false) if (Md->Pl[i].Menu == -1)
   ...
   switch(Md->Pl[i].Item)
    {
      case ITEM_PISTOL        : Use_pistol(i); break;
      case ITEM_MACHINEGUN    : Use_machinegun(i); break;
      case ITEM_BIGMACHINEGUN    : Use_bigmachinegun(i); break;
      case ITEM_LAVATRAP      : Use_lavatrap(i);   break;
      case ITEM_SUPERLAVATRAP : Use_superlavatrap(i);   break;
      case ITEM_MAGNUM        : Use_magnum(i);          break;
      case ITEM_SHOTGUN       : Use_shotgun(i);         break;
      case ITEM_GRENADE       : Use_grenade(i);         break;
      case ITEM_BAZOOKA       : Use_bazooka(i);         break;
      case ITEM_HMISLAUNCHER  : Use_homingmissilelauncher(i);break;
      case ITEM_LASERRIFLE    : Use_laserrifle(i);       break;
      case ITEM_LASERCANNON   : Use_lasercannon(i);       break;
      case ITEM_LASERPISTOL   : Use_laserrifle(i);       break;
      case ITEM_PICKAXE       : Use_pickaxe(i);          break;
      case ITEM_DRILL         : Use_drill(i);            break;
      case ITEM_LAVABOMB      : Use_lavabomb(i);         break;
      case ITEM_SELFDESTRUCTOR: Use_selfdestructor_remote(i);         break;
      case ITEM_PLASMAPISTOL  : Use_plasmapistol(i);     break;
      case ITEM_HEAVYPLASMA   : Use_heavyplasma(i);     break;
      case ITEM_LASERSABRE    : Use_lasersabre(i);      break;
 ...
 if (Md->Pl[i].Menu != -1) // jos pelaaja on menussa
  {
   int Vanhamenu = Md->Pl[i].Menu;
    ...
    if (Md->Pl[i].Menu == MENU_BUYPROJECTW)
     {
      if (Md->Pl[i].Select[Vanhamenu] == 0) {if (Md->Pl[i].Stuff[ITEM_PISTOL] ==0) Buyitem(i, ITEM_PISTOL); else Buyammo(i, ITEM_PISTOL);     }
      if (Md->Pl[i].Select[Vanhamenu] == 1) {if (Md->Pl[i].Stuff[ITEM_MAGNUM] ==0) Buyitem(i, ITEM_MAGNUM); else Buyammo(i, ITEM_MAGNUM);      }
      for (int speeder = 0; speeder < 5; speeder++)
        if (Md->Pl[i].Select[Vanhamenu] == 2) {if (Md->Pl[i].Stuff[ITEM_SHOTGUN] ==0) Buyitem(i, ITEM_SHOTGUN); else Buyammo(i, ITEM_SHOTGUN);    }
      if (Md->Pl[i].Select[Vanhamenu] == 3) {if (Md->Pl[i].Stuff[ITEM_MACHINEGUN] ==0) Buyitem(i, ITEM_MACHINEGUN); else Buyammo(i, ITEM_MACHINEGUN); }
      if (Md->Pl[i].Select[Vanhamenu] == 4) {if (Md->Pl[i].Stuff[ITEM_BIGMACHINEGUN] ==0) Buyitem(i, ITEM_BIGMACHINEGUN); else Buyammo(i, ITEM_BIGMACHINEGUN); }
      if (Md->Pl[i].Select[Vanhamenu] == 5) {if (Md->Pl[i].Stuff[ITEM_BAZOOKA] ==0) Buyitem(i, ITEM_BAZOOKA); else Buyammo(i, ITEM_BAZOOKA);    }
      if (Md->Pl[i].Select[Vanhamenu] == 6) {if (Md->Pl[i].Stuff[ITEM_HMISLAUNCHER] ==0) Buyitem(i, ITEM_HMISLAUNCHER); else Buyammo(i, ITEM_HMISLAUNCHER);}
      if (Md->Pl[i].Select[Vanhamenu] == 7) {if (Md->Pl[i].Stuff[ITEM_SNIPERRIFLE] ==0) Buyitem(i, ITEM_SNIPERRIFLE); else Buyammo(i, ITEM_SNIPERRIFLE);}
     } // if (Md->Pl[i].Menu == MENU_BUYPROJECTW)
    ...
    if (Md->Pl[i].Menu == MENU_BUYAMMO)
     {
      if (Md->Pl[i].Select[Vanhamenu] == 0) {if (Md->Pl[i].Item == ITEM_PISTOL) Buyammo(i, ITEM_PISTOL); else Buyammo(i, ITEM_MACHINEGUN);}
      if (Md->Pl[i].Select[Vanhamenu] == 1)Buyammo(i, ITEM_SHOTGUN);
      if (Md->Pl[i].Select[Vanhamenu] == 2) {if (Md->Pl[i].Item == ITEM_MAGNUM) Buyammo(i, ITEM_MAGNUM);else Buyammo(i, ITEM_SNIPERRIFLE);}
      ...
      if (Md->Pl[i].Select[Vanhamenu] == 6) {if (Md->Pl[i].Item == ITEM_LASERPISTOL) Buyammo(i, ITEM_LASERPISTOL); else Buyammo(i, ITEM_LASERRIFLE);}
      if (Md->Pl[i].Select[Vanhamenu] == 7) {if (Md->Pl[i].Item == ITEM_PLASMAPISTOL) Buyammo(i, ITEM_PLASMAPISTOL); else Buyammo(i, ITEM_HEAVYPLASMA);}
     } // if (Md->Pl[i].Menu == MENU_BUYPROJECTW)
    ...
    if (Md->Pl[i].Menu == MENU_BUYENERGYW)
     {
      if (Md->Pl[i].Select[Vanhamenu] == 0) {if (Md->Pl[i].Stuff[ITEM_LASERSABRE]==0) Buyitem(i, ITEM_LASERSABRE); }
      if (Md->Pl[i].Select[Vanhamenu] == 1) {if (Md->Pl[i].Stuff[ITEM_LASERPISTOL]==0) Buyitem(i, ITEM_LASERPISTOL);else Buyammo(i, ITEM_LASERPISTOL);}
      if (Md->Pl[i].Select[Vanhamenu] == 2) {if (Md->Pl[i].Stuff[ITEM_LASERRIFLE]==0) Buyitem(i, ITEM_LASERRIFLE); else Buyammo(i, ITEM_LASERRIFLE);  }
      if (Md->Pl[i].Select[Vanhamenu] == 3) {if (Md->Pl[i].Stuff[ITEM_LASERCANNON]==0) Buyitem(i, ITEM_LASERCANNON); else Buyammo(i, ITEM_LASERCANNON);  }
      if (Md->Pl[i].Select[Vanhamenu] == 4) {if (Md->Pl[i].Stuff[ITEM_PLASMAPISTOL]==0) Buyitem(i, ITEM_PLASMAPISTOL); else Buyammo(i, ITEM_PLASMAPISTOL);}
      if (Md->Pl[i].Select[Vanhamenu] == 5) {if (Md->Pl[i].Stuff[ITEM_HEAVYPLASMA]==0) Buyitem(i, ITEM_HEAVYPLASMA); else Buyammo(i, ITEM_HEAVYPLASMA);}
     } // if (Md->Pl[i].Menu == MENU_BUYENERGYW)
...
void PlayerChange(int i)
 ...
 // poistumisnappi... poistuu erilaisista menuista
   if (Md->Pl[i].Menu == -1) Md->Pl[i].Menu = MENU_MAIN; else
    {
...
*/
     ...
     if (key[SxF8]) {Takescrshot((PALETTE) Dat[DATPALETTE].dat); key[SxF8] = 0;}
    } // for (int i = 0; i < Opt->Players; i++)
 } // Checkkeys
